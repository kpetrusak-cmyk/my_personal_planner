...
export function HabitTracker() {
  ...
  const toggle = async (habitIndex: number, dateKey: string) => {
    ...
    await supabase.from("habit_tracker").upsert(
      { user_id: user.id, habit_index: habitIndex, habit_name: habits[habitIndex].name, habit_color: habits[habitIndex].color, date_key: dateKey, done: newVal, updated_at: new Date().toISOString() },
